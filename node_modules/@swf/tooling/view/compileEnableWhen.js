...
const compile = ( node, context ) => {
    ...
    let exprEnable = node.getAttribute( Attr );
    if( exprEnable ) {
        exprEnable = exprEnable.replace( /(\r\n|\n|\r)/gm, '' ).trim();
        node.removeAttribute( Attr );
        node.setAttribute( Attr + '-hoc', `{{${exprEnable}}}` );
    }
