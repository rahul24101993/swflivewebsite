...
/**
 * aw-long-press Node Compiler
 */
const {
    Node
} = require( './utils' );

const Attr = 'aw-long-press';
...
const compile = ( node, context ) => {
    let exprPress = node.getAttribute( Attr );
    if( exprPress ) {
        exprPress = exprPress.replace( /(\r\n|\n|\r)/gm, '' ).trim();
        node.removeAttribute( Attr );
        node.setAttribute( Attr + '-hoc', `{{${exprPress}}}` );
    }
