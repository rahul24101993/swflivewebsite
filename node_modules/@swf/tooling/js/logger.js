 ...
 */
function logIt( level, prefix, msg ) {
    wlogger.log( {
        level: level,
        message: msg || '',
        prefix: prefix || ''
...
// Ensure we're working with the minmal node.js version
if( process.versions.node.split( '.' )[ 0 ] < 16 ) {
    exports.error( `Node.js ${process.versions.node} isn't support. Node.js 16.x required!` );
    process.exit( 1 );
}

// Useful node debugging
exports.debug( `Node.js version ${process.versions.node}` );
exports.silly( `NODE_OPTIONS=${process.env.NODE_OPTIONS || ''}` );
exports.debug( `node heap limit = ${require( 'v8' ).getHeapStatistics().heap_size_limit / ( 1024 * 1024 )} Mb` );
exports.debug( `max buffer size = ${require( 'buffer' ).kMaxLength / ( 1024 * 1024 )} Mb` );
