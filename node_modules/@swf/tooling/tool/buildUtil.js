 ...
 * @return {Stream} stream
 */
module.exports.tapBlock = function( func ) {
    const files = [];
...
module.exports.copyFile = async function copyFile( src, dest ) {
    ...
    await module.exports.stream2Promise( gulp.src( src, { dot: true, follow: true } )
        ...
        .pipe( chmod( 0o777 ) )
        .pipe( gulp.dest( dest ) ) );
    logger.info( `File copy from ${logger.pathColor( src )} to ${logger.pathColor( shortenPath( dest ) )} successful.` );
};
 ...
 */
module.exports.merge = function( obj1, obj2, check, depth ) {
    if( !depth ) { depth = 1; }
    if( _.isNull( obj2 ) ) {
        // nothing to merge
