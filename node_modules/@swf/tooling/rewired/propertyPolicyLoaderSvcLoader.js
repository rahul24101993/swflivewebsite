...
/**
 * main function of property policy loader
 * @param {string} source input file content
 * @returns {string} modified source as return value
 */
module.exports = function loader( source ) {
    const stopwatch = new Stopwatch( 'propertyPolicyLoaderSvcLoader' );
    ...
    const res = [];
    res.push( 'AFX_PROPERTY_POLICY_REGISTRATION' );
    ...
    const requiredPaths = _.map( sourcePaths, ( modules, srcPath ) =>
        `require.context('${normalizePath( srcPath )}',true,/(${modules.join( '|' )})[\\\\/]src[\\\\/](assets[\\\\/])?policies[\\\\/][^\\\\/]*PropertyPolicy\\.json$/, 'lazy' )` );
    res.push( `    registerPropertyPolicies( [
        ${requiredPaths.join( ',' )}
    ] );` );

    const output = source.replace( 'AFX_PROPERTY_POLICY_REGISTRATION', res.join( '\n' ) );
    stopwatch.end();
    return output;
};
